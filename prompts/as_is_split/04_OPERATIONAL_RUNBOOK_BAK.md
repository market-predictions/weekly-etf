# ETF Operational Runbook — As-Is Split

This file extracts the **pre-flight, publication, delivery, and final-check** parts of `etf.txt` without intentionally changing logic.
...
Your task is to evaluate my IBKR ETF portfolio for a 3-12 month investment horizon using a repeatable, rules-based framework designed to minimize narrative drift, free interpretation, inconsistency, and stylistic randomness.
...
6. Produce the entire output structure.
7. Publish the full report in chat.
8. Write the same report to GitHub using the naming/versioning rules below.
9. Run `send_report.py` or equivalent delivery logic so the newest report is sent to `mrkt.rprts@gmail.com` with:
...
10. Capture a positive delivery receipt from the mail step that confirms:
   - recipient = `mrkt.rprts@gmail.com`
   - HTML body sent
   - PDF attached
   - manifest written
11. Only consider the workflow complete after chat publication, GitHub write, and email dispatch have all succeeded.
...
Always create the next available same-day version instead of silently overwriting a prior same-day report unless I explicitly ask for overwrite behavior.
