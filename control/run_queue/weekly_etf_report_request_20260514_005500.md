...
requested_at_utc: 2026-05-14T00:55:00Z
...
## Purpose
This run validates the link-aware Dutch PDF-audit cleanup fix after the previous run reached Dutch language quality validation and failed on residual linked ticker wording such as `SMH remains`.
...
`runtime/scrub_nl_pdf_audit_leaks.py` is now markdown-link-aware for ticker verb phrases. It should normalize both plain and linked variants, for example:

- `SMH remains`
- `[SMH](...) remains`
- `GLD remains`
- `[GLD](...) remains`
- `<TICKER> remains the cleanest growth expression`
- `<TICKER> remains the leading funded growth exposure`
- `<TICKER> remains a hedge review`
- `<TICKER> remains structurally valid`
- `<TICKER> creates`
- `<TICKER> must justify itself`
- `<TICKER> must be proven`

The markdown link must be preserved while the surrounding client-facing sentence is localized.
