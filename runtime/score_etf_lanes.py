...
def price_tier(symbol: str, context: LaneContext) -> str:
    return str(context.price_tier_by_symbol.get(symbol.upper()) or "")


def has_valuation_grade_price(symbol: str, context: LaneContext) -> bool:
    symbol = symbol.upper()
    return context.price_status_by_symbol.get(symbol) in PRICED_CLOSE_STATUSES and price_tier(symbol, context) == VALUATION_GRADE


def pricing_confidence(symbol: str, context: LaneContext) -> str:
    symbol = symbol.upper()
    status = context.price_status_by_symbol.get(symbol)
    tier = price_tier(symbol, context)
    if status in EXACT_CLOSE_STATUSES and tier == VALUATION_GRADE:
        return "fresh_exact_valuation_grade_priced"
    if status in PRICED_CLOSE_STATUSES and tier == VALUATION_GRADE:
        return "prior_valid_valuation_grade_priced"
    if status in EXACT_CLOSE_STATUSES and tier == RESEARCH_GRADE:
        return "fresh_exact_research_grade_only"
    if status in PRICED_CLOSE_STATUSES and tier == RESEARCH_GRADE:
        return "prior_valid_research_grade_only"
    if status in PRICED_CLOSE_STATUSES:
        return "priced_but_tier_unclassified"
...
def fundability_status(lane: dict[str, Any], context: LaneContext) -> str:
    primary = str(lane.get("primary_etf", "")).upper()
    alt = str(lane.get("alternative_etf", "")).upper()
    challenger = is_challenger(lane, context)
    if primary in context.held_tickers or alt in context.held_tickers:
        return "held_or_overlap_not_new_funding_candidate"
    if not challenger:
        return "research_radar_only"
    if has_valuation_grade_price(primary, context):
        return "funding_candidate_valuation_grade"
    if context.price_status_by_symbol.get(primary) in PRICED_CLOSE_STATUSES:
        return "research_only_requires_valuation_grade_price"
    return "pricing_missing_for_funding_candidate"
...
def adjusted_lane_score(lane: dict[str, Any], context: LaneContext) -> tuple[float, float, str]:
    ...
    if primary_conf == "fresh_exact_valuation_grade_priced":
        price_bonus = 0.05
    elif primary_conf == "prior_valid_valuation_grade_priced":
        price_bonus = 0.02
    elif primary_conf.endswith("research_grade_only"):
        price_bonus = -0.01
    elif primary_conf == "priced_but_tier_unclassified":
        price_bonus = -0.02
    else:
        price_bonus = -0.03
    novelty = novelty_status(lane, context)
...
def _rejection_reason(lane: dict[str, Any]) -> str:
    ...
    funding_status = str(lane.get("fundability_status", ""))
    r1 = lane.get("return_1m_pct")
    r3 = lane.get("return_3m_pct")
    rs3 = lane.get("rs_vs_spy_3m_pct")

    if funding_status == "research_only_requires_valuation_grade_price":
        return "Structurally relevant and priced, but only research-grade pricing is available; it cannot be funded yet."
    if funding_status == "pricing_missing_for_funding_candidate":
        return "Structurally relevant, but valuation-grade challenger pricing is missing; keep as discovery candidate."
    if "not_in_current_pricing_audit" in primary_status and r1 is None:
        return "Structurally relevant, but neither current audit pricing nor historical momentum was available; keep as discovery candidate."
...
def score_lane(lane: dict[str, Any], context: LaneContext) -> dict[str, Any]:
    ...
    challenger = is_challenger(lane, context)
    novelty = novelty_status(lane, context)
    funding_status = fundability_status(lane, context)
    ...
    output.update(
        {
            ...
            "fundability_status": funding_status,
            "is_fundable_candidate": funding_status == "funding_candidate_valuation_grade",
            "primary_price_status": primary_status,
            "alternative_price_status": alt_status,
            "primary_pricing_tier": price_tier(primary, context) or "not_priced",
            "alternative_pricing_tier": price_tier(alt, context) if alt else "not_applicable",
            "primary_pricing_source": context.price_source_by_symbol.get(primary),
            "alternative_pricing_source": context.price_source_by_symbol.get(alt) if alt else None,
...
def apply_promotion_flags(scored: list[dict[str, Any]], promoted: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for lane in scored:
        ...
        if item["promoted_to_live_radar"] and item.get("challenger") is True and item.get("fundability_status") != "funding_candidate_valuation_grade":
            item["promotion_fundability_note"] = "Promoted to radar only; funding requires valuation-grade pricing."
