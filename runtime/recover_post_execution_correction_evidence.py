...
def _correction_output_paths(report_path: Path) -> tuple[Path, ...]:
    return (
        report_path,
...
def _snapshot_outputs(request) -> dict[Path, bytes | None]:
    return {
...
def _reject_historical_output_changes(snapshot: dict[Path, bytes | None]) -> None:
    changed = [
        path
        for path, previous in snapshot.items()
        if previous is not None and (not path.exists() or path.read_bytes() != previous)
    ]
    if changed:
        _restore_outputs(snapshot)
        names = ", ".join(relative(path) for path in changed)
        raise RuntimeError(
            "Recovery would overwrite existing historical report evidence; original bytes were restored. "
            f"Changed: {names}"
        )
...
def main() -> None:
    ...
    output_snapshot = _snapshot_outputs(request)
    ...
    try:
        ...
        _reject_historical_output_changes(output_snapshot)
        ...
        print(
            ...
            "email_send=false | state_immutable=true | ledger_immutable=true | historical_outputs_preserved=true"
        )
    except Exception:
        _restore_outputs(output_snapshot)
