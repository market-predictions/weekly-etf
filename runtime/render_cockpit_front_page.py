...
def _resolve_runtime_state_path(output_dir: Path, explicit_runtime_state: str | None = None) -> Path:
    ...
    return _output_relative(Path(_read_text_pointer(pointer)), output_dir)
...
def _first_number(mapping: dict[str, Any], keys: Iterable[str], default: float = 0.0) -> float:
    """Return the first present numeric field, preserving authoritative zero values."""
    for key in keys:
...
def _fmt_pct(value: float, language: str, signed: bool = False, decimals: int = 1) -> str:
...
def _market_value_eur(position: dict[str, Any]) -> float:
    return _first_number(position, ("market_value_eur", "previous_market_value_eur"))
...
def _previous_weight(position: dict[str, Any]) -> float:
    return _first_number(position, ("previous_weight_pct", "weight_inherited_pct", "current_weight_pct"))


def _total_nav(state: dict[str, Any]) -> float:
    portfolio = state.get("portfolio") or {}
    explicit = _first_number(portfolio, ("total_portfolio_value_eur", "nav_eur"))
    if explicit > 0:
        return explicit
    return round(_float(portfolio.get("cash_eur")) + sum(_market_value_eur(row) for row in _positions(state)), 2)
...
def _macro_surface(macro: dict[str, Any]) -> tuple[str, int]:
    ...
    memory = macro.get("regime_memory") or {}
    current = str(regime.get("current") or memory.get("current_regime") or "Regime not available")
    confidence = int(round(_float(regime.get("confidence") if "confidence" in regime else memory.get("current_confidence"), 0.0) * 100))
    return current, max(0, min(confidence, 100))
...
def _executed_actions(state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for position in _positions(state):
        ...
        if abs(delta) <= 1e-9 and action.lower() in {"", "none", "no", "n/a"}:
            continue
        actions.append(
            {
                ...
                "delta": delta,
                "action": action,
                "previous_weight": _previous_weight(position),
...
def _action_surface(state: dict[str, Any], language: str) -> tuple[str, str]:
    ...
    if not actions:
        if language == "nl":
            return "Geen portefeuilleactie", "Geen positie is in deze run geopend, gesloten, vergroot of verkleind."
        return "No portfolio action", "No position was opened, closed, increased, or reduced in this run."
    ...
    for item in actions:
        ...
        delta = item["delta"]
        if delta < 0 or action in {"sell", "reduce", "reduced", "close", "closed"}:
            verb = "afgebouwd" if language == "nl" else "reduced"
        elif delta > 0 or action in {"buy", "add", "added", "open", "opened"}:
            verb = "toegevoegd" if language == "nl" else "added"
        else:
            verb = "aangepast" if language == "nl" else "adjusted"
        labels.append(f"{item['ticker']} {verb}")
        details.append(
            f"{item['ticker']} {_fmt_pct(item['previous_weight'], language)} → "
            f"{_fmt_pct(item['current_weight'], language)}"
        )
    separator = " · "
    note_separator = "; "
    return separator.join(labels), note_separator.join(details) + "."
...
def _review_tickers(state: dict[str, Any]) -> list[str]:
    ...
    for position in _positions(state):
        ...
        alternative = str(position.get("better_alternative_exists") or "").lower()
        if ticker and ("review" in fresh_cash or "under" in fresh_cash or "review" in role or alternative == "yes" or _position_weight(position) > SOFT_MAX_POSITION_PCT):
...
def _discipline_surface(state: dict[str, Any], language: str) -> str:
    ...
    if language == "nl":
        if weight > SOFT_MAX_POSITION_PCT:
            return f"Belangrijkste disciplinepunt: {ticker} is {_fmt_pct(weight, 'nl')} en blijft boven de zachte positielimiet; nieuw kapitaal blijft geblokkeerd totdat de disciplinepoorten vrijgeven."
        if reviews:
    ...
    if weight > SOFT_MAX_POSITION_PCT:
        return f"Main discipline point: {ticker} is {_fmt_pct(weight, 'en')} and remains above the soft position cap; fresh capital stays blocked until discipline gates clear."
    if reviews:
...
def _plain_summary(state: dict[str, Any], macro: dict[str, Any], points: list[tuple[str, float]], language: str) -> str:
    ...
    if language == "nl":
        return f"De portefeuille staat sinds de start op {_fmt_pct(since, 'nl', signed=True)}, het marktklimaat is {escape(regime)}, en de grootste concentratie is {ticker} met {_fmt_pct(weight, 'nl')}. Actie deze week: {action_title}."
    return f"The portfolio is {_fmt_pct(since, 'en', signed=True)} since inception, the market climate is {escape(regime)}, and the largest concentration is {ticker} at {_fmt_pct(weight, 'en')}. This week's action: {action_title}."
...
def _sparkline_svg(points: list[tuple[str, float]]) -> str:
    ...
    coords = [(left + index / max(len(points) - 1, 1) * plot_width, top + (1 - (value - minimum) / span) * plot_height) for index, (_, value) in enumerate(points)]
    polyline = " ".join(f"{x:.1f},{y:.1f}" for x, y in coords)
    return f'<svg class="spark" viewBox="0 0 {width} {height}" role="img" aria-label="Equity curve preview"><polyline points="{polyline}" fill="none" stroke="#0F4438" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/><circle cx="{coords[-1][0]:.1f}" cy="{coords[-1][1]:.1f}" r="4.2" fill="#0F4438"/></svg>'
...
def _source_evidence_html(output_dir: Path, state_path: Path, macro_path: Path, language: str) -> str:
    ...
    manifest_path = _optional_pointer_target(output_dir, RUN_MANIFEST_POINTER)
    ...
    if language == "nl":
        ...
        labels = [
            ...
            ("Run-manifest referentie", _relative_label(manifest_path, output_dir) if manifest_path and manifest_path.exists() else "niet beschikbaar"),
        ...
        labels = [
            ...
            ("Run-manifest reference", _relative_label(manifest_path, output_dir) if manifest_path and manifest_path.exists() else "not available"),
    ...
    return f'<div class="section-title">{escape(title)}</div><section class="evidence-strip" data-source-evidence="true" data-promotion-status="not_promoted"><p>{escape(intro)}</p><div class="evidence-grid">{rows}</div></section>'


CSS = """
:root{--paper:#F6F1E7;--paper-2:#EFE8D9;--ink:#211C16;--ink-soft:#5A5043;--line:#D8CDB8;--petrol:#0F4438;--brass:#B07D2B;--pos:#2F7A57;--neg:#A8452C}*{box-sizing:border-box}body{margin:0;padding:28px 16px;background:var(--paper);color:var(--ink);font-family:Arial,Helvetica,sans-serif}.sheet{max-width:780px;margin:0 auto;border:1px solid var(--line);background:var(--paper);box-shadow:0 18px 48px -28px rgba(33,28,22,.45)}.pad{padding:34px 42px 28px}.top{display:flex;justify-content:space-between;gap:20px;border-bottom:2px solid var(--ink);padding-bottom:14px}.kicker,.issue,.strap,.section-title,.lab,.ms,.foot,.ei-label,.ei-value{font-family:'Courier New',monospace}.kicker{font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:var(--brass)}.mast{font-family:Georgia,'Times New Roman',serif;font-size:40px;font-weight:700;line-height:1;margin-top:8px}.mast em{color:var(--petrol);font-style:italic;font-weight:400}.issue{text-align:right;font-size:10.5px;line-height:1.65;color:var(--ink-soft)}.strap{margin-top:12px;font-size:10px;letter-spacing:.13em;text-transform:uppercase;color:var(--ink-soft)}.section-title{margin:28px 0 12px;color:var(--petrol);font-size:11px;letter-spacing:.19em;text-transform:uppercase;border-bottom:1px solid var(--line);padding-bottom:7px}.lede{font-family:Georgia,'Times New Roman',serif;font-size:20px;line-height:1.48}.row{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:24px}.card{border:1px solid var(--line);background:var(--paper-2);padding:16px 18px}.lab{font-size:9.5px;letter-spacing:.17em;text-transform:uppercase;color:var(--ink-soft);margin-bottom:9px}.value{font-family:Georgia,'Times New Roman',serif;font-size:20px;font-weight:600;line-height:1.25}.note{font-size:12px;color:var(--ink-soft);line-height:1.45;margin-top:9px}.bar{height:6px;background:#E2D9C6;border-radius:4px;overflow:hidden;margin-top:12px}.bar i{display:block;height:100%;background:var(--petrol)}.perf-wrap{border:1px solid var(--line);background:#fff}.chartbox{padding:18px}.chart-cap{display:flex;justify-content:space-between;gap:16px}.spark{width:100%;height:120px}.metrics{display:grid;grid-template-columns:repeat(3,1fr);border-top:1px solid var(--line)}.metric{padding:15px 16px;border-right:1px solid var(--line);border-top:1px solid var(--line)}.metric:nth-child(3n){border-right:none}.metric:nth-child(-n+3){border-top:none}.ml{font-family:'Courier New',monospace;font-size:9px;text-transform:uppercase;color:var(--ink-soft)}.mv{font-family:Georgia,'Times New Roman',serif;font-size:24px;font-weight:600;margin-top:7px}.pos{color:var(--pos)}.neg{color:var(--neg)}.ms{font-size:10.5px;color:var(--ink-soft);margin-top:5px}.discipline{border-left:4px solid var(--brass);background:rgba(176,125,43,.10);padding:14px 16px;font-size:13px;line-height:1.5}.evidence-strip{border:1px solid var(--line);background:#fff;padding:14px 16px}.evidence-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 14px}.evidence-item{border-top:1px solid var(--line);padding-top:8px}.ei-label{font-size:9px;text-transform:uppercase;color:var(--ink-soft)}.ei-value{margin-top:4px;font-size:10px;overflow-wrap:anywhere}.foot{margin-top:22px;padding-top:13px;border-top:1px solid var(--line);display:flex;justify-content:space-between;color:var(--ink-soft);font-size:10px}@media print{body{padding:0}.sheet{box-shadow:none;border:none}}
"""
...
def _html_document(state: dict[str, Any], macro: dict[str, Any], points: list[tuple[str, float]], language: str, output_dir: Path, state_path: Path, macro_path: Path) -> str:
    ...
    evidence = _source_evidence_html(output_dir, state_path, macro_path, language)
    html = f'''<!DOCTYPE html><html lang="{language}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>{escape(text['title'])} — Cockpit preview</title><style>{CSS}</style></head><body><div class="sheet" data-cockpit-front-page="true" data-preview-only="true"><div class="pad"><header class="top"><div><div class="kicker">{escape(text['kicker'])}</div><div class="mast">{escape(text['title']).replace('ETF', '<em>ETF</em>')}</div></div><div class="issue">Model portfolio · EUR<br>{escape(_fmt_date(report_date, language))}<br><b>Preview lane</b></div></header><div class="strap">Weekly · US ETF · {escape(text['preview'])}</div><div class="section-title">{escape(text['short'])}</div><p class="lede">{summary}</p><div class="row"><div class="card"><div class="lab">{escape(text['climate'])}</div><div class="value">{escape(regime)}</div><div class="bar"><i style="width:{confidence}%"></i></div><div class="note">{confidence}% {escape(text['confidence'])}</div></div><div class="card"><div class="lab">{escape(text['action'])}</div><div class="value">{escape(action_title)}</div><div class="note">{escape(action_note)}</div></div></div><div class="section-title">{escape(text['perf'])}</div><div class="perf-wrap"><div class="chartbox"><div class="chart-cap"><span>{escape(text['nav'])} · EUR</span><span>{escape(_fmt_eur(nav, language))} · <b>{escape(_fmt_pct(since, language, signed=True))}</b></span></div>{_sparkline_svg(points)}</div><div class="metrics"><div class="metric"><div class="ml">{escape(text['ret'])}</div><div class="mv {'pos' if since >= 0 else 'neg'}">{escape(_fmt_pct(since, language, signed=True))}</div></div><div class="metric"><div class="ml">{escape(text['dd'])}</div><div class="mv {'neg' if drawdown < 0 else ''}">{escape(_fmt_pct(drawdown, language))}</div></div><div class="metric"><div class="ml">Cash</div><div class="mv">{escape(_fmt_pct(cash_pct, language))}</div><div class="ms">{escape(_fmt_eur(cash, language))}</div></div><div class="metric"><div class="ml">{escape(text['positions'])}</div><div class="mv">{len(_positions(state))}</div></div><div class="metric"><div class="ml">{escape(text['largest'])}</div><div class="mv">{escape(largest_ticker)}</div><div class="ms">{escape(_fmt_pct(largest_weight, language))}</div></div><div class="metric"><div class="ml">EUR/USD</div><div class="mv">{_float((state.get('fx_basis') or {}).get('rate')):.4f}</div></div></div></div><div class="section-title">{escape(text['disc'])}</div><div class="discipline">{escape(discipline)}</div>{evidence}<div class="foot"><span>Preview-only artifact generated from current runtime state.</span><span>01 — cockpit preview</span></div></div></div></body></html>'''
    leaked = [token for token in INTERNAL_SURFACE_TOKENS if token in html.lower()]
    if leaked:
