...
REPORT_STATE_POINTER = Path("output/runtime/latest_etf_report_state_path.txt")
...
def _resolve_runtime_state_path(output_dir: Path, explicit_runtime_state: str | None = None) -> Path:
    if explicit_runtime_state:
        path = Path(explicit_runtime_state)
        return path if path.is_absolute() else output_dir.parent / path if str(path).startswith("output/") else path
    pointer = output_dir / "runtime" / "latest_etf_report_state_path.txt"
    target = _read_text_pointer(pointer)
    state_path = Path(target)
    return state_path if state_path.is_absolute() else output_dir.parent / state_path if target.startswith("output/") else output_dir / state_path


def _output_relative(path: Path, output_dir: Path) -> Path:
    return path if path.is_absolute() else output_dir.parent / path if str(path).startswith("output/") else output_dir / path


def _load_macro_pack(output_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
    source_files = state.get("source_files") or {}
    raw_path = source_files.get("macro_policy_pack") or str(MACRO_POLICY_PACK_PATH)
    path = _output_relative(Path(raw_path), output_dir)
    if path.exists():
        try:
...
def _total_nav(state: dict[str, Any]) -> float:
    ...
    if explicit > 0:
        return explicit
    cash = _float(portfolio.get("cash_eur"), 0.0)
    return round(cash + sum(_market_value_eur(position) for position in _positions(state)), 2)
...
def _next_sequence(preview_dir: Path, token: str, language: str) -> int:
    ...
    existing = []
    for path in preview_dir.glob(f"{prefix}_{token}_*.html"):
...
def _action_surface(state: dict[str, Any], language: str) -> tuple[str, str]:
    ...
    for position in _positions(state):
        ...
        if abs(delta) > 1e-9:
            changed.append(f"{str(position.get('ticker')).upper()} {delta:+.2f} shares")
    if language == "nl":
        return "Portefeuilleactie", "; ".join(changed) if changed else "Actie aanwezig volgens runtime state."
    return "Portfolio action", "; ".join(changed) if changed else "Action present in runtime state."
...
def _review_tickers(state: dict[str, Any]) -> list[str]:
    ...
    for position in _positions(state):
        ...
        better = str(position.get("better_alternative_exists") or "").lower() == "yes"
        overweight = _position_weight(position) > SOFT_MAX_POSITION_PCT
        if "review" in fresh_cash or "under" in fresh_cash or "herbeoordeling" in fresh_cash or "review" in role or better or overweight:
...
def _plain_summary(state: dict[str, Any], macro: dict[str, Any], points: list[tuple[str, float]], language: str) -> str:
    ...
    if language == "nl":
        return (
            f"We houden deze week discipline boven activiteit. De portefeuille staat sinds de start op {_fmt_pct(since, 'nl', signed=True)}, "
            f"het marktklimaat blijft {escape(regime)}, en de grootste concentratie zit in {ticker} met {_fmt_pct(weight, 'nl')}. "
            f"Actie deze week: {action_title.lower()}."
        )
    return (
        f"We keep discipline ahead of activity this week. The portfolio is {_fmt_pct(since, 'en', signed=True)} since inception, "
        f"the market climate remains {escape(regime)}, and the largest concentration is {ticker} at {_fmt_pct(weight, 'en')}. "
        f"This week's action: {action_title.lower()}."
    )
...
def _sparkline_svg(points: list[tuple[str, float]]) -> str:
    ...
    for idx, (_, value) in enumerate(points):
        x = left + (idx / max(len(points) - 1, 1)) * plot_w
        y = top + (1 - ((value - min_v) / span)) * plot_h
        coords.append((x, y))
    poly = " ".join(f"{x:.1f},{y:.1f}" for x, y in coords)
    baseline_y = top + (1 - ((STARTING_CAPITAL_EUR - min_v) / span)) * plot_h
    return f"""
<svg class="spark" viewBox="0 0 {width} {height}" role="img" aria-label="Equity curve preview">
  <line x1="{left}" y1="{baseline_y:.1f}" x2="{width-right}" y2="{baseline_y:.1f}" stroke="#D8CDB8" stroke-width="1" stroke-dasharray="2 4" />
  <polyline points="{poly}" fill="none" stroke="#0F4438" stroke-width="3" stroke-linejoin="round" stroke-linecap="round" />
  <circle cx="{coords[-1][0]:.1f}" cy="{coords[-1][1]:.1f}" r="4.2" fill="#0F4438" />
</svg>
""".strip()


def _html_document(state: dict[str, Any], macro: dict[str, Any], points: list[tuple[str, float]], language: str) -> str:
    is_nl = language == "nl"
    nav = _total_nav(state)
    ...
    positions_count = len(_positions(state))
...
<title>{escape(text['title'])} — Cockpit preview</title>
<style>
  :root {{
    --paper:#F6F1E7; --paper-2:#EFE8D9; --ink:#211C16; --ink-soft:#5A5043;
    --line:#D8CDB8; --petrol:#0F4438; --brass:#B07D2B; --pos:#2F7A57; --neg:#A8452C;
  }}
  * {{ box-sizing:border-box; }}
  body {{ margin:0; padding:28px 16px; background:var(--paper); color:var(--ink); font-family:Arial, Helvetica, sans-serif; }}
  .sheet {{ max-width:780px; margin:0 auto; border:1px solid var(--line); background:var(--paper); box-shadow:0 18px 48px -28px rgba(33,28,22,.45); }}
  .pad {{ padding:34px 42px 28px; }}
  .top {{ display:flex; justify-content:space-between; gap:20px; border-bottom:2px solid var(--ink); padding-bottom:14px; }}
  .kicker,.issue,.strap,.section-title,.lab,.ms,.foot {{ font-family:'Courier New', monospace; }}
  .kicker {{ font-size:10px; letter-spacing:.18em; text-transform:uppercase; color:var(--brass); }}
  .mast {{ font-family:Georgia, 'Times New Roman', serif; font-size:40px; font-weight:700; line-height:1; margin-top:8px; letter-spacing:-.02em; }}
  .mast em {{ color:var(--petrol); font-style:italic; font-weight:400; }}
  .issue {{ text-align:right; font-size:10.5px; line-height:1.65; color:var(--ink-soft); }}
  .issue b {{ color:var(--ink); }}
  .strap {{ margin-top:12px; display:flex; gap:12px; align-items:center; font-size:10px; letter-spacing:.13em; text-transform:uppercase; color:var(--ink-soft); }}
  .dot {{ width:5px; height:5px; border-radius:50%; background:var(--brass); display:inline-block; }}
  .section-title {{ margin:28px 0 12px; color:var(--petrol); font-size:11px; letter-spacing:.19em; text-transform:uppercase; display:flex; align-items:center; gap:12px; }}
  .section-title:after {{ content:""; height:1px; background:var(--line); flex:1; }}
  .lede {{ font-family:Georgia, 'Times New Roman', serif; font-size:20px; line-height:1.48; }}
  .row {{ display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-top:24px; }}
  .card {{ border:1px solid var(--line); background:var(--paper-2); padding:16px 18px; }}
  .lab {{ font-size:9.5px; letter-spacing:.17em; text-transform:uppercase; color:var(--ink-soft); margin-bottom:9px; }}
  .value {{ font-family:Georgia, 'Times New Roman', serif; font-size:20px; font-weight:600; line-height:1.25; }}
  .note {{ font-size:12px; color:var(--ink-soft); line-height:1.45; margin-top:9px; }}
  .conf {{ display:flex; align-items:center; gap:9px; margin-top:12px; }}
  .bar {{ flex:1; height:6px; background:#E2D9C6; border-radius:4px; overflow:hidden; }}
  .bar i {{ display:block; height:100%; width:{max(0, min(confidence, 100))}%; background:linear-gradient(90deg,var(--petrol),#1c6b56); }}
  .perf-wrap {{ border:1px solid var(--line); background:#fff; }}
  .chartbox {{ padding:18px 18px 6px; }}
  .chart-cap {{ display:flex; justify-content:space-between; align-items:baseline; gap:16px; margin-bottom:6px; }}
  .chart-cap .t {{ font-family:'Courier New', monospace; font-size:10px; letter-spacing:.12em; text-transform:uppercase; color:var(--ink-soft); }}
  .chart-cap .v {{ font-family:Georgia, 'Times New Roman', serif; font-size:16px; }}
  ...
  .metric {{ padding:15px 16px; border-right:1px solid var(--line); border-top:1px solid var(--line); }}
  .metric:nth-child(3n) {{ border-right:none; }}
  .metric:nth-child(-n+3) {{ border-top:none; }}
  .ml {{ font-family:'Courier New', monospace; font-size:9px; letter-spacing:.12em; text-transform:uppercase; color:var(--ink-soft); }}
  .mv {{ font-family:Georgia, 'Times New Roman', serif; font-size:24px; font-weight:600; margin-top:7px; line-height:1; }}
  .mv.pos {{ color:var(--pos); }} .mv.neg {{ color:var(--neg); }}
  .ms {{ font-size:10.5px; color:var(--ink-soft); margin-top:5px; }}
  .discipline {{ border-left:4px solid var(--brass); background:rgba(176,125,43,.10); padding:14px 16px; font-size:13px; line-height:1.5; }}
  .foot {{ margin-top:22px; padding-top:13px; border-top:1px solid var(--line); display:flex; justify-content:space-between; gap:12px; color:var(--ink-soft); font-size:10px; }}
  @media print {{ body {{ padding:0; }} .sheet {{ box-shadow:none; border:none; }} }}
</style>
</head>
<body>
<div class="sheet" data-cockpit-front-page="true" data-preview-only="true">
  <div class="pad">
    <header class="top">
      <div>
        <div class="kicker">{escape(text['kicker'])}</div>
        <div class="mast">{escape(text['title']).replace('ETF', '<em>ETF</em>')}</div>
      </div>
      <div class="issue">{escape(text['model'])}<br>{escape(_fmt_date(report_date, language))}<br><b>Preview lane</b></div>
    </header>
    <div class="strap"><span>{escape(text['frequency'])}</span><span class="dot"></span><span>US ETF</span><span class="dot"></span><span>{escape(text['preview'])}</span></div>

    <div class="section-title">{escape(text['short'])}</div>
    <p class="lede">{summary}</p>

    <div class="row">
      <div class="card">
        <div class="lab">{escape(text['climate'])}</div>
        <div class="value">{escape(regime)}</div>
        <div class="conf"><div class="bar"><i></i></div><span>{confidence}% {escape(text['confidence'])}</span></div>
      </div>
      <div class="card">
        <div class="lab">{escape(text['action'])}</div>
        <div class="value">{escape(action_title)}</div>
        <div class="note">{escape(action_note)}</div>
      </div>
    </div>

    <div class="section-title">{escape(text['perf'])}</div>
    <div class="perf-wrap">
      <div class="chartbox">
        <div class="chart-cap"><span class="t">{escape(text['nav'])} · EUR</span><span class="v">{escape(_fmt_eur(nav, language))} · <b>{escape(_fmt_pct(since, language, signed=True))}</b></span></div>
        {_sparkline_svg(points)}
      </div>
      <div class="metrics">
        <div class="metric"><div class="ml">{escape(text['ret'])}</div><div class="mv {'pos' if since >= 0 else 'neg'}">{escape(_fmt_pct(since, language, signed=True))}</div><div class="ms">{escape(_fmt_eur(points[0][1], language))} → {escape(_fmt_eur(nav, language))}</div></div>
        <div class="metric"><div class="ml">{escape(text['dd'])}</div><div class="mv {'neg' if drawdown < 0 else ''}">{escape(_fmt_pct(drawdown, language))}</div><div class="ms">since inception</div></div>
        <div class="metric"><div class="ml">{escape(text['cash'])}</div><div class="mv">{escape(_fmt_pct(cash_pct, language))}</div><div class="ms">{escape(_fmt_eur(cash, language))}</div></div>
        <div class="metric"><div class="ml">{escape(text['positions'])}</div><div class="mv">{positions_count}</div><div class="ms">active holdings</div></div>
        <div class="metric"><div class="ml">{escape(text['largest'])}</div><div class="mv">{escape(largest_ticker)}</div><div class="ms">{escape(_fmt_pct(largest_weight, language))}</div></div>
        <div class="metric"><div class="ml">EUR/USD</div><div class="mv">{_float((state.get('fx_basis') or {}).get('rate')):.4f}</div><div class="ms">pricing basis</div></div>
      </div>
    </div>

    <div class="section-title">{escape(text['disc'])}</div>
    <div class="discipline">{escape(discipline)}</div>

    <div class="foot"><span>Preview-only artifact generated from current runtime state.</span><span>01 — cockpit preview</span></div>
  </div>
</div>
</body>
</html>"""
...
def render_cockpit_front_page(
    *,
    output_dir: Path,
    language: str = "both",
    html_only: bool = False,
    runtime_state: str | None = None,
) -> list[RenderedCockpit]:
    ...
    for lang in languages:
        ...
        html = _html_document(state, macro, points, lang)
...
def main() -> None:
    args = _parse_args()
    rendered = render_cockpit_front_page(
