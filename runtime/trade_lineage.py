...
def _derive_previous_values(item: dict[str, Any], previous_shares: float) -> None:
    current_shares = _float(item.get("shares"))
    if current_shares <= SHARE_TOLERANCE:
        return
    for current_key, previous_key in (
        ("market_value_local", "previous_market_value_local"),
        ("market_value_eur", "previous_market_value_eur"),
    ):
        current_value = _float(item.get(current_key))
        if current_value >= 0:
            item[previous_key] = round(current_value / current_shares * previous_shares, 2)
...
def normalize_trade_lineage_rows(
    ...
    *,
    ...
    Official ledger rows are preferred. Legacy rows whose previous fields were overwritten
    with post-trade values are repaired deterministically from the recorded share and weight
    deltas. The function only changes the supplied copies; callers decide whether to persist.
    """
    ...
    for raw in rows:
        ...
        previous_shares = max(0.0, current_shares - shares_delta)
        item["previous_shares"] = round(previous_shares, 6)
        ...
        if ledger:
            previous_weight = max(0.0, _float(ledger.get("previous_weight_pct")))
            ledger_new_weight = _float(ledger.get("new_weight_pct"), current_weight)
            item["weight_change_pct"] = round(ledger_new_weight - previous_weight, 4)
            ...
            previous_weight = max(0.0, current_weight - recorded_change) if overwritten else max(0.0, existing_previous)
            item["trade_lineage_source"] = (
                "derived_from_recorded_delta" if overwritten else "preserved_pre_trade_snapshot"
            )

        item["previous_weight_pct"] = round(previous_weight, 4)
        item["weight_inherited_pct"] = round(previous_weight, 4)
        _derive_previous_values(item, previous_shares)
        normalized.append(item)
    return normalized
...
def validate_trade_lineage_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        previous_shares = _float(row.get("previous_shares"), float("nan"))
        shares_delta = _float(row.get("shares_delta_this_run", row.get("shares_delta")))
        if previous_shares != previous_shares:
            errors.append(f"{context}:missing_previous_shares:{ticker}")
        elif abs((current_shares - previous_shares) - shares_delta) > max(SHARE_TOLERANCE, abs(shares_delta) * 1e-6):
            errors.append(
                f"{context}:share_delta_mismatch:{ticker}:"
                f"{previous_shares:.6f}->{current_shares:.6f}!={shares_delta:.6f}"
            )
        ...
        implied_change = current_weight - previous_weight
        ...
        if material_change and abs(implied_change - recorded_change) > 0.15:
            errors.append(
                ...
                f"{previous_weight:.4f}->{current_weight:.4f}!={recorded_change:.4f}"
            )
        if material_change and round(previous_weight, display_decimals) == round(current_weight, display_decimals):
            errors.append(
                f"{context}:material_trade_identical_display_weight:{ticker}:"
                f"{previous_weight:.4f}->{current_weight:.4f}"
            )
        if current_shares > SHARE_TOLERANCE and row.get("previous_market_value_eur") in (None, ""):
            errors.append(f"{context}:missing_previous_market_value_eur:{ticker}")
    return errors
