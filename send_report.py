...
def _has_pdf_equity_curve_embed(html_pdf: str) -> bool:
    lowered = html_pdf.lower()
    return (
        "data:image/png;base64," in lowered
        or '<img src="' in lowered
        or "equity-curve-svg-block" in lowered
        or ("<svg" in lowered and "equity curve" in lowered)
        or ("<svg" in lowered and "portefeuillecurve" in lowered)
    )
...
def generate_delivery_assets(output_dir: Path, report_path: Path, mode: str = "standard", language: str = "en"):
    ...
    if "EQUITY_CURVE_CHART_PLACEHOLDER" in md_text_clean and equity_curve_png.exists():
        if not _has_pdf_equity_curve_embed(html_pdf):
